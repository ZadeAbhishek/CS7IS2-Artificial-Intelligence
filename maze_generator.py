...
Generates a maze with enforced walls on the outer border and designated
entry (1, 0) and exit (maze.shape[0]-2, maze.shape[1]-1).
"""
...
def enforce_borders(maze):
    """
    Force the outer boundary of the maze to be walls (value = 1).
    """
    maze[0, :] = 1
    maze[-1, :] = 1
    maze[:, 0] = 1
...
def create_maze_dfs(dim):
    """
    Creates a maze using DFS (recursive backtracking).
    The result is a 'perfect' maze with a single unique path from start to goal.
    """
    ...
    # Designate entry (1,0) and exit (bottom-right).
    maze[1, 0] = 0
    maze[-2, -1] = 0

    # Enforce outer border walls, then re-open entry & exit.
...
def create_maze_prims(dim, extra_openings=0):
    """
    Creates a maze using Prim's algorithm, optionally with extra openings to add loops.
    """
...
def create_maze_aldous_broder(n, m, complexity=0):
    """
    Creates a maze using the Aldous-Broder algorithm, which tends to produce many loops.
    """
    ...
    # Add random openings for complexity.
...
def generate_maze(difficulty=5, dim=20):
    ...
    - difficulty <= 3 -> DFS
    - 4..6 -> Prim's (with extra openings)
    - >= 7 -> Aldous-Broder (with complexity)
    """
    if difficulty <= 3:
...
if __name__ == "__main__":
    ...
    plot_maze(maze, title=f"Maze ({dim}x{dim} cells, Difficulty: {difficulty})")